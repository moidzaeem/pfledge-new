...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('pflegeForm').addEventListener('submit', function (e) {
      ...
      if (pflegegrad === 1) {
        sachleistung = 0;
        pflegegeld = 0;
        pflegevollstationaer = 125;
      } else if (pflegegrad === 2) {
        sachleistung = 761;
        pflegegeld = 332;
        tagesnachtpflege = 689;
  ...
  // ###################### INDEX 2 PASSWORD INPUT EYE ########################
  document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.index2TogglePassword').forEach(function(toggleButton) {
      toggleButton.addEventListener('click', function() {
        // Find the closest container (the parent div)
        const container = toggleButton.closest('.index2-password-container');
        // Then select the input inside that container
        const passwordInput = container.querySelector('.index2PasswordInput');
  ...
  // ###################### REGISTRATION 2 STEPPER FORM ########################
  let currentStep = 0;

  function updateButtons() {
      const prevButton = document.getElementById("registration2PrevBtn");
      const nextButton = document.getElementById("registration2NextBtn");
      let steps = document.querySelectorAll(".registration2-step");
  
      prevButton.classList.toggle("registration2-hidden", currentStep === 0);
      nextButton.classList.toggle("registration2-full-width", currentStep === 0);
  
      // Remove previous event listener to prevent duplicates
      nextButton.removeEventListener("click", showPopup);
  
      if (currentStep === steps.length - 1) {
          nextButton.textContent = "Jetzt lospflegen";
          nextButton.classList.add("stepper-popup-btn");
  
          // Attach event listener ONLY at the last step
          nextButton.addEventListener("click", showPopup);
      } else {
          nextButton.textContent = "Weiter";
          nextButton.classList.remove("stepper-popup-btn");
      }
  }
  ...
  function nextStep() {
      let steps = document.querySelectorAll(".registration2-step");
      let progress = document.querySelectorAll(".registration2-progress-bar div");
  
      if (currentStep < steps.length - 1) {
          steps[currentStep].classList.add("registration2-active");
          progress[currentStep].classList.add("registration2-active");
  
          steps[currentStep].classList.remove("registration2-active2");
          progress[currentStep].classList.remove("registration2-active2");
  
          currentStep++; // Move to the next step FIRST
  
          steps[currentStep].classList.add("registration2-active", "registration2-active2");
          progress[currentStep].classList.add("registration2-active", "registration2-active2");
  
          updateButtons(); // Update the button AFTER moving to the next step
      }
  }
  ...
  function prevStep() {
      ...
      if (currentStep > 0) {
          ...
          steps[currentStep].classList.remove("registration2-active");
          progress[currentStep].classList.remove("registration2-active");
  
          currentStep--;
  
          steps[currentStep].classList.add("registration2-active2");
          progress[currentStep].classList.add("registration2-active2");
  ...
  document.addEventListener("DOMContentLoaded", function () {
      const neinRadio = document.querySelector('input[name="registration22-options"][value="option33"]');
      const jaRadio = document.querySelector('input[name="registration22-options"][value="option11"]');
      const step2Cont1 = document.querySelector(".registration2-step2-cont1");
      const step2Cont2 = document.querySelector(".registration2-step2-cont2");
  
      if (neinRadio && jaRadio && step2Cont1 && step2Cont2) {
          neinRadio.addEventListener("change", function () {
              step2Cont1.style.display = this.checked ? "none" : "block";
              step2Cont2.style.display = this.checked ? "block" : "none";
          });
  
          jaRadio.addEventListener("change", function () {
              step2Cont1.style.display = this.checked ? "block" : "none";
              step2Cont2.style.display = this.checked ? "none" : "block";
          });
      }
  
      let steps = document.querySelectorAll(".registration2-step");
      if (steps.length > 0) {
          steps[0].classList.add("registration2-active");
      }
 ...
 // ###################### REGISTRATION 2 STEPPER FORM RANGE SLIDER #######################

 document.addEventListener("DOMContentLoaded", function () {
    const rangeSlider = document.querySelector("#stepper-range-slider");
    const rangeSliderThumb = document.querySelector("#stepper-range-thumb");
    const rangeSliderProgress = document.querySelector("#stepper-range-progress");

    // Function to update the slider progress and thumb
    function updateSlider() {
        let percent = ((rangeSlider.value - rangeSlider.min) / (rangeSlider.max - rangeSlider.min)) * 100;

        // Move the thumb & update the value inside it
        rangeSliderThumb.style.left = `${percent}%`;
        rangeSliderThumb.textContent = rangeSlider.value;

        // Update the progress bar width
        rangeSliderProgress.style.width = `${percent}%`;
    }
...
document.addEventListener("DOMContentLoaded", function () {
  $(document).ready(function() {
      var table = $('#userTable').DataTable({
          ...
          "lengthMenu": [6, 8, 10], // Dropdown options
          "paging": true, // Enable pagination
          "info": false, // Hide "Showing X to Y of Z entries"
          "searching": false, // Disable search bar
          "ordering": false, // Disable sorting completely
          "stripeClasses": [], // Remove alternating row colors
          "language": {
              "lengthMenu": "Show result: _MENU_", // Custom dropdown label
              "paginate": {
                  "first": "«",
                  "last": "»",
                  "next": "›",
          ...
          // Wrap the table in a container with class "table-wrapper"
          "dom": '<"table-wrapper"t><"bottom d-flex justify-content-between align-items-center"l p>'
      });

      // Remove background colors from all TDs
      $('#userTable tbody td').css({
          "background-color": "transparent"
      });

      // Ensure no row has any background color
      $('<style>')
          .prop('type', 'text/css')
          .html(`
              #userTable tbody tr {
...
// ############################# ADMIN USER POPUP ########################
document.addEventListener("DOMContentLoaded", function () {
    document.getElementById("adminUserPopupBtn").addEventListener("click", function () {
      var myModal12 = new bootstrap.Modal(document.getElementById("adminUserPopup"));
      myModal12.show();
    });
  });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // ############################# MEIN PROFIL POPUP1 ########################
document.addEventListener("DOMContentLoaded", function () {
  document.getElementById("meinProfilPopupIcon1").addEventListener("click", function () {
    var myModal12 = new bootstrap.Modal(document.getElementById("meinProfilPopup1"));
    myModal12.show();
  });
});
  ...
  // ############################# MEIN PROFIL POPUP2 ########################
document.addEventListener("DOMContentLoaded", function () {
  document.getElementById("meinProfilPopupIcon2").addEventListener("click", function () {
    var myModal12 = new bootstrap.Modal(document.getElementById("meinProfilPopup2"));
    myModal12.show();
  });
});
  // ############################# MEIN PROFIL POPUP3 ########################
document.addEventListener("DOMContentLoaded", function () {
  document.getElementById("meinProfilPopupIcon3").addEventListener("click", function () {
    var myModal12 = new bootstrap.Modal(document.getElementById("meinProfilPopup3"));
    myModal12.show();
  });
});
  ...
  // ############################# MEIN PROFIL POPUP4 ########################
document.addEventListener("DOMContentLoaded", function () {
  document.getElementById("meinProfilPopupIcon4").addEventListener("click", function () {
    var myModal12 = new bootstrap.Modal(document.getElementById("meinProfilPopup4"));
    myModal12.show();
  });
});
